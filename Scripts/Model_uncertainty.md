...
sa_ctroids2 <- sa_ctroids1 %>% mutate(NAME =
                                        case_when(NAME == "BOLIVIA" ~ "BO", 
                                                  NAME == "PARAGUAY" ~ "PY",
                                                  NAME == "URUGUAY" ~ "UY",
                                                  NAME == "ARGENTINA" ~ "AR",
                                                  NAME == "BRAZIL" ~ "BR"))
...
p18 <- ggplot() +
  ...
  theme(legend.key.size = unit(2, 'line'), # Change legend key size
        ...
        legend.title.align=0.5) + # Change legend text font size
...
ggsave(plot = p18, "./Plots/Continuous models/Model_SD.png", width = 8.3, height = 11.7)
...
p19 <- ggplot() +
  ...
  theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b =0, l = 0), size = 22),
        ...
        axis.text.y = element_text(colour = "black", size = 18)) +
  theme(legend.position = c(0.8, 0.25)) +
  theme(legend.key.height = unit(1.5, 'line'), # Change legend key height
...
ggsave(plot = p19, "./Plots/Continuous models/Model_range.png", width = 8.3, height = 11.7)
